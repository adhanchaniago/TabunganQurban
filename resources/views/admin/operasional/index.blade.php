        ...
        <div class="card">
            ...
            <div class="card-block">
                <div id="nestable-menu" class="m-b-10">
                    {!! link_to_route(config('quickadmin.route').'.operasional.create', trans('quickadmin::templates.templates-view_index-add_new') , null, array('class' => 'btn btn-success')) !!}
                </div>


                    @if ($operasional->count())
...
<td>{{ $row->tanggal_pengeluaran }}</td>
                            ...
                            <td>
                                {!! link_to_route(config('quickadmin.route').'.operasional.edit', trans('quickadmin::templates.templates-view_index-edit'), array($row->id), array('class' => 'btn btn-sm btn-info')) !!}
                                {!! Form::open(array('style' => 'display: inline-block;', 'method' => 'DELETE', 'onsubmit' => "return confirm('".trans("quickadmin::templates.templates-view_index-are_you_sure")."');",  'route' => array(config('quickadmin.route').'.operasional.destroy', $row->id))) !!}
            ...
            {!! Form::open(['route' => config('quickadmin.route').'.operasional.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
