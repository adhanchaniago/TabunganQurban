        ...
        <div class="card">
            ...
            <div class="card-block">
                <div id="nestable-menu" class="m-b-10">
                    {!! link_to_route(config('quickadmin.route').'.belanjapakan.create', trans('quickadmin::templates.templates-view_index-add_new') , null, array('class' => 'btn btn-success')) !!}
                </div>


                    @if ($belanjapakan->count())
...
<td>{{ $row->tanggal_pembelian }}</td>
                            ...
                            <td>
                                {{--{!! link_to_route(config('quickadmin.route').'.belanjapakan.edit', trans('quickadmin::templates.templates-view_index-edit'), array($row->id), array('class' => 'btn btn-sm btn-info')) !!}--}}
                                {!! Form::open(array('style' => 'display: inline-block;', 'method' => 'DELETE', 'onsubmit' => "return confirm('".trans("quickadmin::templates.templates-view_index-are_you_sure")."');",  'route' => array(config('quickadmin.route').'.belanjapakan.destroy', $row->id))) !!}
            ...
            {!! Form::open(['route' => config('quickadmin.route').'.belanjapakan.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
