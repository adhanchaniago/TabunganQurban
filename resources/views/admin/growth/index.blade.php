        ...
        <div class="card">
            ...
            <div class="card-block">
                <div id="nestable-menu" class="m-b-10">
                    {!! link_to_route(config('quickadmin.route').'.growth.create', trans('quickadmin::templates.templates-view_index-add_new') , null, array('class' => 'btn btn-success')) !!}
                </div>
                ...
                <tbody>
                    @foreach ($growth as $row)
                        <tr>
                            ...
                            <td>{{ isset($row->belanjasapi->eartag) ? $row->belanjasapi->eartag : '' }}</td>
<td>{{ $row->bobot }}</td>
<td>{{ $row->tanggal_cek }}</td>

                            <td>
                                {!! link_to_route(config('quickadmin.route').'.growth.edit', trans('quickadmin::templates.templates-view_index-edit'), array($row->id), array('class' => 'btn btn-sm btn-info')) !!}
                                {!! Form::open(array('style' => 'display: inline-block;', 'method' => 'DELETE', 'onsubmit' => "return confirm('".trans("quickadmin::templates.templates-view_index-are_you_sure")."');",  'route' => array(config('quickadmin.route').'.growth.destroy', $row->id))) !!}
            ...
            {!! Form::open(['route' => config('quickadmin.route').'.growth.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
